...
class SettingsStore extends SettingsStoreClient {
    ...
    constructor(namespace, default_settings, offscreenDocumentProvider) {
        super(namespace);
        ...
        this.onStorageInitialized = new Promise(resolve => {
            ...
            chrome.storage.sync.get(null).then(async (items) => {
    ...
    async initOrMigrateSettings(offscreenDocumentProvider, default_settings) {
    ...
    calculateDisplayBasedDefaultSettings() {
    ...
    async cleanLocalStorageFormData() {
    ...
    async fromObject(values, merge = false) {

    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        chrome.tabs.query({ currentWindow: true, active: true }, async function (tabs) {
            try {
                ...
                const res = popupQueryBGResponse = await chrome.runtime.sendMessage({ method: '[AutomaticTabCleaner:popupQuery]', tab: currentTab });
                    ...
                    if (windows.hasOwnProperty(wi)) {
                        ...
                        for (const j in windows[wi].tabs)
                            if (windows[wi].tabs.hasOwnProperty(j)) {
                                ...
                                // @ts-ignore
                                const divLine = drawPreviewTile(tabMeta, {
                                    noTime: true,
                                    //close: true,
                                    noHref: true,
                                    noTitle: true,

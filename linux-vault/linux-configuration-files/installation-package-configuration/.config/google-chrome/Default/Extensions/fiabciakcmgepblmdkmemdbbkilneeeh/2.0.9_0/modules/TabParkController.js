...
function parkTabs(requestTab, windowId) {
    const callbackSingle = async function (window) {
        let number = 0;
    ...
    const callbackAll = async function (windows) {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function parkTabGroup(tab) {
    if (tab == null || tab.groupId === -1) {
        console.warn('Cannot suspend tab group: tab is not in a group');
        return;
    }
    const groupId = tab.groupId;
    chrome.windows.get(tab.windowId, { 'populate': true }, async function (window) {
        let number = 0;
        for (const j in window.tabs) {
            if (window.tabs.hasOwnProperty(j)) {
        ...
        if (screenExist == null || parseInt(screenExist) <= 0) {
            ...
            const parkByMessage = function (closureTab, closureTabId) {
                chrome.windows.get(closureTab.windowId, async function (win) {
                    ...
                    chrome.tabs.sendMessage(closureTabId, {
        ...
        for (const j in window.tabs) {
            if (window.tabs.hasOwnProperty(j)) {
                ...
                if (currentTab.groupId === groupId && TabManager.isTabParked(currentTab)) {
                    const tmpFunction = function (currentTab) {
                        const clzOpenedIndex = openedIndex++;
                        setTimeout(function () {
...
//# sourceMappingURL=TabParkController.js.map
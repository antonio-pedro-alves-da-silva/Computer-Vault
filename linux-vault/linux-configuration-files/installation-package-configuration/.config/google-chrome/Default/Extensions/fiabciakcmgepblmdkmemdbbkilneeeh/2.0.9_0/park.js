...
void (async () => {
    ...
    try {
        ...
        screenPromise = withTimeout(chrome.runtime.sendMessage({ method: '[TS:getScreen]', tabId, sessionId: searchParams.get('sessionId') }), 2500, { scr: null, pixRat: null } // fallback: screenshot unavailable
        );
        const parkData = await chrome.runtime.sendMessage({ method: '[TS:dataForParkPage]', tabId, sessionId: searchParams.get('sessionId') }); //.then((parkData: ParkPageDataBGResponse) => {

...
class TabManager {
    tabInfos = {};
    TAB_INFO_CLEANUP_TTL_MS = 60 * 60 * 24 * 1000; /* 24 Hours */
    COMMON_LOOP_PERIOD_MS = 10000; /* 10 Sec */
    historyOpenerController = new HistoryOpenerController();
    commonInterval;
    /* For puppeteer tests only */
    setTabInfoCleanupTtlMs(ttlMs) {
        this.TAB_INFO_CLEANUP_TTL_MS = ttlMs;
    }
    /* For puppeteer tests only */
    setCommonLoopPeriodMs(commonLoopPeriodMs) {
    ...
    async compress(str, encoding = 'gzip') {
    ...
    loadTabInfos() {
        ...
        chrome.storage.local.get(this.TAB_INFOS_KEY).then(async (result) => {
            const loadedITabInfosCompressedBase64 = result[self.TAB_INFOS_KEY];
            if (loadedITabInfosCompressedBase64 != null && loadedITabInfosCompressedBase64.trim().length > 0) {
                ...
                const loadedITabInfos = JSON.parse(iTabInfosJson);
    ...
    constructor() {
                ...
                try {
                    ...
                    if (sessionId != null && tabId != null) {
                        ...
                        getScreenCache = {
                            sessionId: sessionId,
                            tabId: tabId,
                            getScreenPromise: new Promise((resolve) => {
        ...
        chrome.tabs.onActivated.addListener(function (activeInfo) {
            ...
            processedPromise.then(async (tab) => {
                //if (debug) {
                ...
                try {
                    if ( /*isTabURLAllowedForPark(tab) &&*/!TabManager.isTabParked(tab) && tab.url.indexOf(sessionsPageUrl) == -1) {
                        if (!tab.discarded)
                            (function (closureTab) {
                                setTimeout(function () {
                                    void tabCapture.captureTab(closureTab, { checkActiveTabNotChanged: true });
                                }, 400);
                            })(tab);
                    }
                }
    ...
    onTabReplaceDetected(addedTabId, removedTabId) {
        if (trace)
    ...
    getTabInfoOrCreate(tab) {
        ...
        if (tabInfo == null && tab.url.startsWith(parkUrl)) {
            ...
            if (this.tabInfos[removedTabId] == null) {
                ...
                const restoredTabInfo = new TabInfo({ id: removedTabId, url: originalUrl });
    ...
    markTabParkedFromInject(tabId) {
        const tabInfo = this.getTabInfoById(tabId);
        if (tabInfo != null)
            tabInfo.parked = true;
    }
    setTabUnsuspended(tab) {
        const tabInfo = this.getTabInfoOrCreate(tab);
        tabInfo.time = 0;
        tabInfo.suspended_time = 0;
        tabInfo.parkTrys = 0;
    }
    setLastCaptureUrl(tab) {
        const tabInfo = this.getTabInfoOrCreate(tab);
        tabInfo.lstCapUrl = tab.url;
        tabInfo.lstCapTime = Date.now();
    }
    calculateAndMarkClosedTabs(openedChromeTabs) {
    ...
    ;
    static CHROME_STORE_URL_1 = 'https://chrome.google.com/webstore';
    static CHROME_STORE_URL_2 = 'https://chromewebstore.google.com';
    /*** Static ***/
    static isTabURLAllowedForPark(tab) {
        return (tab.url.substring(0, tab.url.indexOf(':')) == 'http' ||
            (tab.url.substring(0, tab.url.indexOf(':')) == 'https' && (tab.url.indexOf(TabManager.CHROME_STORE_URL_1) < 0
                && tab.url.indexOf(TabManager.CHROME_STORE_URL_2) < 0)) ||
            tab.url === wizardPageUrl);
    }
    static canTabBeScripted(tab) {

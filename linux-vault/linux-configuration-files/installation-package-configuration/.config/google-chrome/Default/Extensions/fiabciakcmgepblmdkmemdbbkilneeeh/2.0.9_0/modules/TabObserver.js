...
class TabObserver {
    ...
    async start() {
        ...
        if (await settings.get('active'))
            TabObserver.ticker = setInterval(() => { self.tick().catch(console.error); }, TabObserver.tickSize * 1000);
    }
    settingsChanged() {
        this.start().catch(console.error);
        this.tick(true).catch(console.error);
    }
    async tick(stateOnly) {
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const self = this;
        /* TODO-v3:
                                        ...
                                        try {
                                            if (TabManager.canTabBeScripted(tab)) {
                                                chrome.scripting.executeScript({
                                                    target: { tabId: tabId },
                                                    func: (title, tabInfo) => {
                                                        const iTabInfo = tabInfo;
                                                        function appendTitleDebug(title, iTabInfo) {
                                                        ...
                                                        function debugInfoString(iTabInfo) {
...
if (typeof global !== "undefined")
    global.TabObserver = TabObserver;
//# sourceMappingURL=TabObserver.js.map